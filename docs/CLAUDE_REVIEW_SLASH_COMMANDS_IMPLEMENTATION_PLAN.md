...
- [ ] **3.1.4** セキュリティテストケースの定義
  - [ ] 3.1.4.1 パストラバーサル攻撃
  - [ ] 3.1.4.2 コマンドインジェクション
  - [ ] 3.1.4.3 不正な入力値

#### 3.2 ユニットテスト実装 (claude-review.sh) (0/12)

- [ ] **3.2.1** 正常系テスト
  - [ ] 3.2.1.1 デフォルト引数でのレビュー実行
  - [ ] 3.2.1.2 カスタムタイムアウトでのレビュー実行
  - [ ] 3.2.1.3 特定コミットのレビュー実行
  - [ ] 3.2.1.4 カスタム出力ディレクトリでのレビュー実行

- [ ] **3.2.2** 異常系テスト
  - [ ] 3.2.2.1 引数なしでのエラー処理
  - [ ] 3.2.2.2 存在しないコミットでのエラー処理
  - [ ] 3.2.2.3 不正なタイムアウト値でのエラー処理
  - [ ] 3.2.2.4 書き込み権限のないディレクトリでのエラー処理

- [ ] **3.2.3** 境界値テスト
  - [ ] 3.2.3.1 タイムアウト0秒
  - [ ] 3.2.3.2 タイムアウト最小値（1秒）
  - [ ] 3.2.3.3 タイムアウト最大値（3600秒）
  - [ ] 3.2.3.4 空のコミット

- [ ] **3.2.4** 外部依存テスト
  - [ ] 3.2.4.1 gitコマンド未インストール時のエラー処理
  - [ ] 3.2.4.2 Claude MCP未設定時のフォールバック処理
  - [ ] 3.2.4.3 ネットワーク切断時のエラー処理

#### 4.3 ユニットテスト実装 (claude-security-review.sh) (0/12)

- [ ] **3.3.1** 正常系テスト
  - [ ] 3.3.1.1 SQLインジェクション検出
  - [ ] 3.3.1.2 XSS脆弱性検出
  - [ ] 3.3.1.3 コマンドインジェクション検出
  - [ ] 3.3.1.4 ハードコードされた秘密情報検出

- [ ] **3.3.2** 異常系テスト
  - [ ] 3.3.2.1 セキュリティルールファイル不在時のエラー処理
  - [ ] 3.3.2.2 不正な形式のコード入力でのエラー処理
  - [ ] 3.3.2.3 タイムアウト発生時の処理
  - [ ] 3.3.2.4 出力ディレクトリ作成失敗時の処理

- [ ] **3.3.3** 境界値テスト
  - [ ] 3.3.3.1 脆弱性0件のコード
  - [ ] 3.3.3.2 脆弱性1件のコード
  - [ ] 3.3.3.3 脆弱性100件以上のコード
  - [ ] 3.3.3.4 最大サイズのコードファイル

- [ ] **3.3.4** セキュリティテスト
  - [ ] 3.3.4.1 パストラバーサル攻撃の検出と防御
  - [ ] 3.3.4.2 コマンドインジェクション攻撃の検出と防御
  - [ ] 3.3.4.3 不正なファイルパス入力の拒否
  - [ ] 3.3.4.4 環境変数インジェクションの防御

#### 4.4 統合テスト実装 (0/4)

- [ ] **3.4.1** CLIスクリプト統合テスト
  - [ ] 3.4.1.1 claude-review.sh の実行確認
  - [ ] 3.4.1.2 claude-security-review.sh の実行確認
  - [ ] 3.4.1.3 両スクリプトの連続実行
  - [ ] 3.4.1.4 出力ファイルの整合性確認

- [ ] **3.4.2** E2Eテスト
  - [ ] 3.4.2.1 実際のコミットに対するレビュー実行
  - [ ] 3.4.2.2 セキュリティ脆弱性を含むコードのレビュー
  - [ ] 3.4.2.3 レポート生成の確認
  - [ ] 3.4.2.4 VibeLoggerログの確認

- [ ] **3.4.3** パフォーマンステスト
  - [ ] 3.4.3.1 小規模コードベースでのレビュー時間測定
  - [ ] 3.4.3.2 大規模コードベースでのレビュー時間測定
  - [ ] 3.4.3.3 並列実行時のリソース使用量測定

- [ ] **3.4.4** カバレッジ測定
  - [ ] 3.4.4.1 ユニットテストカバレッジ測定（kcovまたはbashcov）
  - [ ] 3.4.4.2 分岐網羅率の確認（目標: 100%）
  - [ ] 3.4.4.3 未カバー箇所の特定と追加テスト作成

### Phase 4: ドキュメント作成 (0/8)

- [ ] **4.1** README.md更新
  - [ ] 4.1.1 CLIスクリプトのセクション追加
  - [ ] 4.1.2 使用例の記述
  - [ ] 4.1.3 トラブルシューティング情報
...
| Phase | タスク数 | 完了 | 進捗率 |
|-------|---------|------|--------|
| Phase 1: 準備・設計 | 8 | 2 | 25% |
| Phase 2: スクリプト実装 | 26 | 26 | 100% |
| Phase 3: テスト実装 | 32 | 0 | 0% |
| Phase 4: ドキュメント | 8 | 0 | 0% |
