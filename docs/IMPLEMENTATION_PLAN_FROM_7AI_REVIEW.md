...
### P0.2 ユニットテストスイート追加（本番品質ブロッカー）
**推奨元**: Claude CTO (Section 8, Line 645-648), Quality Checker
**見積**: 5-8時間
**ブロッカー**: 本番準備、リグレッション防止

#### P0.2.1 テストフレームワーク選定・セットアップ（1.5時間）
- [ ] **Task P0.2.1.1**: Bashテストフレームワーク評価（1時間）
  - [ ] **bats-core**の評価
    - [ ] インストール: `npm install -g bats`
    - [ ] サンプルテスト実行
    - [ ] CI/CD統合の容易さ確認
    - [ ] モック機能の有無確認
  - [ ] **shunit2**の評価
    - [ ] インストール: `git clone https://github.com/kward/shunit2.git`
    - [ ] サンプルテスト実行
    - [ ] xUnit形式レポート出力確認
  - [ ] **assert.sh**の評価
    - [ ] 軽量性確認
...
- [ ] **Task P0.2.1.2**: 選定フレームワークのセットアップ（30分）
  - [ ] `tests/framework/`ディレクトリ作成
  - [ ] フレームワークのインストール
  - [ ] `tests/run-unit-tests.sh`スクリプト作成
  - [ ] 依存関係ドキュメント作成（`tests/README.md`）
  - [ ] サンプルテスト作成・実行

#### P0.2.2 コアライブラリのユニットテスト（3.5時間）
- [ ] **Task P0.2.2.1**: `multi-ai-core.sh`テスト（1.5時間）
  - [ ] **ロギング関数テスト** (16関数)
    - [ ] `log_info()` - 正常系、境界値
    - [ ] `log_warning()` - 警告メッセージフォーマット
    - [ ] `log_error()` - エラーコード返却確認
    - [ ] `log_success()` - 成功メッセージ
    - [ ] `log_phase_start()` - フェーズ開始ログ
    - [ ] `log_phase_end()` - フェーズ終了ログ
    - [ ] その他10関数
...
- [ ] **Task P0.2.2.3**: `multi-ai-ai-interface.sh`テスト（30分）
  - [ ] **call_ai()モックテスト**
    - [ ] AI CLIコマンドをモック化
    - [ ] 正常応答のシミュレーション
    - [ ] エラー応答のシミュレーション
  - [ ] **call_ai_with_context()ファイル経由テスト**
    - [ ] 小プロンプト（<1KB） → CLI引数使用確認
    - [ ] 中プロンプト（1KB-100KB） → ファイル使用確認
    - [ ] 大プロンプト（>100KB） → ファイル使用確認
    - [ ] 一時ファイル自動削除確認
  - [ ] **フォールバック機構テスト**
    - [ ] プライマリAI失敗 → フォールバックAI使用
    - [ ] 両方失敗 → エラー返却

#### P0.2.3 統合テストの拡充（1.5時間）
- [ ] **Task P0.2.3.1**: 既存テストのカバレッジ分析（30分）
  - [ ] `tests/phase1-file-based-prompt-test.sh`のカバレッジ測定
    - [ ] 現状: ファイルベースプロンプトのみ
    - [ ] 不足: YAML解析、並列実行、エラーハンドリング
  - [ ] `tests/phase4-e2e-test.sh`のカバレッジ測定
    - [ ] 現状: E2Eワークフローのみ
    - [ ] 不足: 個別関数、境界条件

- [ ] **Task P0.2.3.2**: エッジケーステスト追加（1時間）
  - [ ] **極端に大きいプロンプト（>1MB）**
    - [ ] 1MB + 1Bプロンプト → 正常処理確認
    - [ ] メモリ使用量監視
  - [ ] **並列実行の競合状態**
    - [ ] 7AI同時起動 → 一時ファイル衝突なし確認
    - [ ] 並列ログ書き込み → 破損なし確認
  - [ ] **タイムアウト境界条件**
    - [ ] タイムアウト直前完了 → 成功扱い
    - [ ] タイムアウト直後完了 → 失敗扱い

- [ ] **Task P0.2.3.3**: CI/CD統合（実装は後回し、設計のみ）
  - [ ] GitHub Actionsワークフロー設計
  - [ ] テスト自動実行トリガー（PR、push to main）
  - [ ] カバレッジレポート生成
...
### P1.2 YAMLキャッシング実装（パフォーマンス向上）
**推奨元**: Claude CTO (Section 4.3, Line 395-406), Qwen (Line 113-127)
**見積**: 1-2時間
**影響**: 200-400ms削減、5-10%パフォーマンス改善

#### P1.2.1 キャッシングメカニズム実装（1時間）
- [ ] **Task P1.2.1.1**: `multi-ai-config.sh`にキャッシング追加（45分）
  ```bash
  # グローバル連想配列
...
- [ ] **Task P1.2.1.2**: キャッシュキー生成ロジック（15分）
  - [ ] `${config_file}:${yaml_path}:${mtime}`形式
  - [ ] ハッシュ化検討（長いパスの場合）
    ```bash
    cache_key_hash() {
...
- [ ] **Task P1.2.2.1**: 17関数へキャッシング適用
  - [ ] `get_phase_count()` - キャッシュ確認 → なければYAML解析
  - [ ] `get_phase_name()` - 同上
  - [ ] `get_parallel_count()` - 同上
  - [ ] その他14関数

#### P1.2.3 パフォーマンステスト（30分）
- [ ] **Task P1.2.3.1**: ベンチマーク実行
  - [ ] キャッシングなし: ワークフロー実行時間測定
  - [ ] キャッシングあり: 同上
  - [ ] 削減時間の記録（期待: 200-400ms）
